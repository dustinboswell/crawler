...
USAGE = "Usage: %s <file with URLs to fetch> <progress_file> <output file> [<# of concurrent connections>]" % sys.argv[0]
try:
...
class UrlScheduler:
  ...
  def __init__(self):
    ...
    self.domains = Queue.Queue()  # round-robin list (keys into domain_queues)

    # map "domain" -> # times in-a-row next_url(domain) returned url for that domain
  ...
  def is_empty(self):
    return not self.domain_queues  # assmes empty queues disappear instantly
  ...
  def pop_url(self, domain):
    ...
    assert not urls.empty()  # we already make sure empty lists never happen
    self.consecutive_count[domain] = self.consecutive_count.get(domain, 0) + 1
    url = urls.get()
    self.num_urls -= 1
  ...
  def next_url_new_domain(self, not_domains):
    ...
    for i in xrange(num_domains):
      ...
      if not domain in self.domain_queues: continue  # without re-putting
      else: self.domains.put(domain)  # put it back at the end for next round

      if domain in not_domains: continue
...
class Sleeper(object):
  ...
  def delay_secs(self, url):
    ...
    next_time = self.last_fetch_time.get(bucket, 0) + 2.0  # 2 secs from last time.
...
class MrCrawlMulti(curl.MultiFetcher):
  def __init__(self, parallel, output_file):
    ...
    self.robot_manager = robots.RobotsManager(self.useragent, "/crawldata/robots.db")
  ...
  def handle_robots(self, curl):
    if not curl.is_robots_fetch: return False
    curl.is_robots_fetch = False
  ...
  def should_save_page(self, response):
    ...
    if not ctype_lines: return True  # we don't know, err on the side of saving

    content_type = ctype_lines[-1].lower()
    if self.bad_ctype_re.search(content_type): return False
    if self.good_ctype_re.search(content_type): return True
    print content_type
  ...
  def fetch_success(self, curl):
    ...
    if effective_url != curl.url: print effective_url,
    ...
    if not is_robots_txt and self.should_save_page(response):
      ...
      self.output_file.write("mrcrawl_effective_url: %s\n" % effective_url)
  ...
  def do_some_work(self):
    last_domains = [curl.last_domain for curl in self.curls if curl.last_domain]
    ...
    for curl in self.curls:
      if curl.busy: continue
      if curl.sleeping: continue
      ...
      if domain != curl.last_domain:
        if curl.last_domain: self.curl_domains.remove(curl.last_domain)
  ...
  def should_crawl_url(self, url):
    if self.url_filter_re.match(url): return False
    return True
  ...
  def fetch_file(self, url_file):
    ...
    eof = False
    while True:
      if eof and self.url_scheduler.is_empty() and self.num_curls_in_use == 0: break
