...
try:
    opts, args = getopt.getopt(sys.argv[1:], '', ["output-prefix=", "wget-args=", "max-threads="])
...
DOMAIN_SPIDER_PARAMS.update({
    ...
    'domains': '',       # But only stay within these domains (You must override this)
...
def WgetFile(fname, shard_id):
    ...
    cline = "wget %s %s -i %s" % (arg_dict_str(params), arg_dict["wget-args"], fname)
